    ...
    while (1) {
        ...
        switch (idx) {
            case 0:
                ...
                {
                    clear(msgToSend);
                    strcpy(msgToSend, encode("Enter a number denoting your username: ", publicKey, mod).data());
                    ...
                    if (reg[id]) {
                        clear(msgToSend);
                        strcpy(msgToSend, encode("Username already taken", publicKey, mod).data());
                        send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                    } else {
                        reg[id] = 1;
                        clear(msgToSend);
                        strcpy(msgToSend, encode("250 OK", publicKey, mod).data());
                        send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                    }
                }
            ...
            case 1:
                ...
                {
                    clear(msgToSend);
                    strcpy(msgToSend, encode("Enter your email: ", publicKey, mod).data());
                    send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                    ...
                    if (reg[id]) {
                        ...
                        clear(msgToSend);
                        strcpy(msgToSend, encode("Please register", publicKey, mod).data());
                        send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                    }
                }
            ...
            case 2:
                /* Handle message composition */
                {
                    if (!logInId) {
                        clear(msgToSend);
                        strcpy(msgToSend, encode("Please login", publicKey, mod).data());
                        send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                    } else {
                        clear(msgToSend);
                        strcpy(msgToSend, encode("250 OK", publicKey, mod).data());
                        ...
                        while (1) {
                            ...
                            if (ok) {
                                if (!reg[sendId]) {
                                    clear(msgToSend);
                                    strcpy(msgToSend, encode("550 No such user here", publicKey, mod).data());
                                    send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
                                } else {
                                ...
                                clear(msgToSend);
                                strcpy(msgToSend, encode("354 Start mail input; end with X.X", publicKey, mod).data());
                                send(cli.cliFd, msgToSend, sizeof(msgToSend), 0);
...
int main() {
    ...
    while (1) {
        ...
        strcpy(msgToSend, encode("HELLO", publicKey, mod).data());
